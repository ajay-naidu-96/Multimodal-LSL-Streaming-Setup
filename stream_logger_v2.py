#!/usr/bin/env python3
"""
Full LSL Stream Logger with Live Display
Logs all available LSL streams to files with organized folder structure
and shows real-time video/depth display.
"""
...
class StreamLogger:
        ...
        while self.running:
            ...
            if sample:
                try:
                    ...
                    self._update_display_frame(name, depth_colormap)
                    np.save(os.path.join(info['folder'], f'depth_{frame_count}.npy'), depth_array)
                    self.csv_writers[name].writerow({'timestamp': timestamp,'frame_path': f'depth_{frame_count}.npy','min_depth': float(np.min(depth_array)),'max_depth': float(np.max(depth_array)),'mean_depth': float(np.mean(depth_array))})
                    self.csv_files[name].flush()
                    frame_count += 1
                except Exception as e:
                    self._update_stats(name, errors=1)
        ...
        while self.running:
            ...
            if sample:
                try:
                    ...
                    for i, v in enumerate(arr): row[f'audio_sample_{i}'] = v
    ...
    def _log_data_stream(self, name, inlet):
        while self.running:
            ...
            if sample:
                try:
                    row = {'timestamp': timestamp}
                    for i, v in enumerate(sample): row[f'channel_{i}'] = v
                    self.csv_writers[name].writerow(row)
                    self.csv_files[name].flush()
                except Exception as e:
                    self._update_stats(name, errors=1)
    ...
    def stop_logging(self):
        self.running = False
        for t in self.log_threads: t.join(timeout=2.0)
        for f in self.csv_files.values(): f.close()
        for v in self.video_writers.values():
            if 'writer' in v and v['writer']: v['writer'].release()
        for a in self.audio_files.values(): a['file'].close()
        os.system('cls' if os.name == 'nt' else 'clear')

    def run(self, duration: Optional[float] = None):
        if not self.discover_and_connect_streams(): return
        if not self.start_logging(): return
        try:
...
# --- Main ---
def main():
    parser = argparse.ArgumentParser(description='LSL Stream Logger with Live Display')
